...
import { Box, Flex, FlexProps, Text } from '@callisto-enterprise/soy-uikit2'
...
export const PrizePoolRow: React.FC<PrizePoolRowProps> = ({ totalAmount, ...props }) => {
  ...
  return (
    <Row {...props}>
      ...
      <Text bold>{`${getPrizePoolAmount(totalAmount)} CLO`}</Text>
    </Row>
  )
}
  ...
  return (
    <Row height="18px" {...props}>
      ...
      <Flex alignItems="center">
        ...
        <Text mx="4px">|</Text>
        <Text fontSize="12px" lineHeight="18px">{`${formatBnb(amount)} CLO`}</Text>
      </Flex>
    </Row>
  )
}
...
export const PrizePoolHistoryRow: React.FC<PrizePoolHistoryRowProps> = ({ totalAmount, ...props }) => {
  ...
  return (
    <Row {...props}>
      <Text bold>{t('Prize Pool')}:</Text>
      <Text bold>{`${getPrizePoolAmountHistory(totalAmount)} CLO`}</Text>
    </Row>
  )
}
