...
const getButtonProps = (
  ...
  if (!hasSufficientBalance()) {
    return { key: 'Insufficient CLO balance', disabled: true }
  }
...
const SetPositionCard: React.FC<SetPositionCardProps> = ({ position, togglePosition, onBack, onSuccess }) => {
  ...
  useEffect(() => {
    ...
    if (!hasSufficientBalance) {
      setErrorMessage({ key: 'Insufficient CLO balance' })
    } else if (inputAmount.gt(0) && inputAmount.lt(minBetAmount)) {
      setErrorMessage({
        key: 'A minimum amount of %num% %token% is required',
        data: { num: formatBigNumber(minBetAmount), token: `${symbols[chainId]}` },
      })
    } else {
      setErrorMessage(null)
    }
  }, [value, maxBalance, minBetAmount, setErrorMessage, chainId])
