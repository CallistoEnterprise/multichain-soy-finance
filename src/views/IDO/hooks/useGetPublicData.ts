...
const useGetPublicData = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const minPrice =
        (minPricePercentage * getBalanceAmount(new BigNumber(lastRoundSoyPrice.toString())).toNumber()) / 100
      const maxPrice =
        (maxPricePercentage * getBalanceAmount(new BigNumber(lastRoundSoyPrice.toString())).toNumber()) / 100
      ...
      const curDuration =
        parseFloat(defaultDuration.toString()) === 0 ? 0 : (end - start) / parseFloat(defaultDuration.toString())
