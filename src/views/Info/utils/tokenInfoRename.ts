  ...
  if (
    ...
    if (object.token0.address in toRename) {
      if (Object.prototype.hasOwnProperty.call(toRename[object.token0.address], 'symbol')) {
        token0Clone = {
          ...token0Clone,
          symbol: toRename[object.token0.address].symbol,
          name: toRename[object.token0.address].name,
        }
      }
    }
    ...
    if (object.token1.address in toRename) {
      if (Object.prototype.hasOwnProperty.call(toRename[object.token1.address], 'symbol')) {
        token1Clone = {
          ...token1Clone,
          symbol: toRename[object.token1.address].symbol,
          name: toRename[object.token1.address].name,
        }
      }
    }

    return { ...object, token0: token0Clone, token1: token1Clone }
  }
