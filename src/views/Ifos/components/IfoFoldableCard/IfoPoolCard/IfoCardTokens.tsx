...
const CakeBnbTokenSection: React.FC<TokenSectionProps> = (props) => {
  return <TokenSection primaryToken={tokens.soy} secondaryToken={tokens.wclo} {...props} />
}
  ...
  const renderTokenSection = () => {
    ...
    if (account && !hasProfile) {
      if (publicIfoData.status === 'finished') {
        return <Text textAlign="center">{t('Activate soyfinance Profile to take part in next IFO‘s!')}</Text>
      }
      return <Text textAlign="center">{t('You need an active soyfinance Profile to take part in an IFO!')}</Text>
    }
