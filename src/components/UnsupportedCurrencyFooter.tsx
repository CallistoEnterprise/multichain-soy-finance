...
import { Button, Text, Modal, useModal, InjectedModalProps, Link } from '@soy-libs/uikit2'
          ...
          return (
            ...
            Object.keys(unsupportedTokens).includes(token.address) && (
              <AutoColumn key={token.address?.concat('not-supported')} gap="10px">
                ...
                {chainId && (
                  <Link external small color="primaryDark" href={getCallistoExpLink(token.address, 'address', chainId)}>
                    {token.address}
                  </Link>
                )}
              </AutoColumn>
