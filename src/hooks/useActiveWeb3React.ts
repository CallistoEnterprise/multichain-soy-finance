...
const useActiveWeb3React = (): Web3ReactContextInterface<Web3Provider> => {
  ...
  const locChainId = window.localStorage.getItem(localStorageChainIdKey) ? Number(window.localStorage.getItem(localStorageChainIdKey)) : Number(process.env.REACT_APP_CLO_CHAIN_ID)
  useEffect(() => {
