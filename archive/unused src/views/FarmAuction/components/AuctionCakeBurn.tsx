...
import { Text, Flex, Skeleton, Box, Image } from '@callisto-enterprise/soy-uikit2'
  ...
  useEffect(() => {
    const fetchBurnedCakeAmount = async () => {
      try {
        ...
        console.error('Failed to fetch burned auction SOY', error)
      }
    }
  ...
  return (
    <Flex flexDirection={['column-reverse', null, 'row']}>
      <Flex flexDirection="column" flex="2">
        {burnedCakeAmount !== 0 ? (
          <Balance fontSize="64px" bold value={burnedCakeAmount} decimals={0} unit=" SOY" />
        ) : (
      ...
      <Flex flex="1" alignSelf="center">
        <Box width="350px">
          <Image width={350} height={320} src="/images/burnt-cake.png" alt={t('Burnt SOY')} />
        </Box>
      </Flex>
    </Flex>
  )
