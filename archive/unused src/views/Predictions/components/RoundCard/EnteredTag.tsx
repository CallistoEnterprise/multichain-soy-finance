...
import { CheckmarkCircleIcon, CheckmarkCircleFillIcon, Tag, useTooltip } from '@callisto-enterprise/soy-uikit2'
...
const EnteredTag: React.FC<EnteredTagProps> = ({ amount, hasClaimed = false }) => {
  ...
  const { targetRef, tooltipVisible, tooltip } = useTooltip(
    <div style={{ whiteSpace: 'nowrap' }}>{`${formatBnbv2(amount)} CLO`}</div>,
    { placement: 'bottom' },
  )
